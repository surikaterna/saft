      ...
      if (Annotations.hasAnnotation(Provides, mod, k)) {
        ...
        if (fn.length !== 0) {
          if (!Annotations.hasAnnotation(Inject, mod, k)) {
            throw new Error('@Provides function has unsatisfied paramters, @Inject is missing',
              mod, k);
          } else {
            ...
            if (inject.getKeys().length !== fn.length) {
              throw new Error(`@Provides function has unsatisfied paramters, @Inject specifies
                 ${inject.getKeys().length} keys and function expects ${fn.length} parameters`);
            }
            provides.forEach(provide => {
              const t = provide.key;
              binder.bindKey(t).toProvider(fn.bind(mod), inject.getKeys());
            });
          }
        } else {
          provides.forEach(provide => {
            const t = provide.key;
            binder.bindKey(t).toProvider(fn.bind(mod));
          });
        }
      }
    });
